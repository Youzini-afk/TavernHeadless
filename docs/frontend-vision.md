...
- 数据访问：优先使用 TavernHeadless Official Integration Kit，也就是 `@tavern/sdk` 与 `@tavern/client-helpers`。
- 图标：Lucide Vue Next（统一线性风格）。
- 动画：CSS transition + 少量 Motion primitives（仅在状态切换时出现）。

其中，`@tavern/shared` 仍然是内部包，可被仓库内部复用，但不作为前端公开接入面的直接依赖目标来承诺。
...
- `apps/web` 保留页面、组件、交互和本地状态，不再长期维护一套重复的可复用接入层。

如果引擎内部实现、后端路由、SSE 事件、OpenAPI、Tool Calling、MCP 或其他接入方可见语义发生变化，应同步检查：
