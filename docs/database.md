...
阶段一引入 Workspace / Project。阶段二增加 Project Event、Project Membership 和 observer 只读访问。阶段三增加 Derived Output、Project Inbox 和 deriver 成员角色。旧 API 仍保持兼容：
...
Project 成员关系表。当前支持 `owner`、`observer` 和 `deriver` 三种角色。
...
- observer 只能读取 Project、Session、Derived Output 和 `visibility=project` 的 Project Event。
- deriver 可以读取 Project、观察 Project Event、写入 Derived Output、创建 Inbox 条目，并读取自己发送的 Inbox 条目。
- 当前不支持移除 owner，也不支持把同一账号在同一 Project 下从 observer 改成 deriver 或反向修改。需要先移除旧成员关系再重新添加。
...
- deriver 可见 `project` 事件。
- 阶段三新增的事件类型包括 `derived_output.created`、`derived_output.updated`、`derived_output.archived`、`project_inbox.item.created`、`project_inbox.item.accepted`、`project_inbox.item.rejected` 和 `project_inbox.item.archived`。
- Derived Output 和 Inbox 事件负载只携带 ID、状态、类型、来源引用和小型元数据，不携带完整 `value_json` 或 `payload_json`。
- `internal` 事件为内部保留，不对普通 Project 读取接口返回。

## `derived_output`

Project 派生结果表。它保存 deriver 或 owner 写入的 JSON 结果，不会自动合并到主 Session、Variable、Memory 或 Session State。

| 列名 | 类型 | 约束/默认值 | 说明 |
| ---- | ---- | ----------- | ---- |
| `id` | `TEXT` | PK | Derived Output ID，格式 `dout_` 前缀 |
| `workspace_id` | `TEXT` | `NOT NULL`, FK -> `workspace.id`, `ON DELETE RESTRICT` | 所属 Workspace |
| `project_id` | `TEXT` | `NOT NULL`, FK -> `project.id`, `ON DELETE RESTRICT` | 所属 Project |
| `account_id` | `TEXT` | `NOT NULL`, FK -> `account.id`, `ON DELETE RESTRICT` | Project owner 账号 ID |
| `owner_account_id` | `TEXT` | `NOT NULL`, FK -> `account.id`, `ON DELETE RESTRICT` | 写入该派生结果的账号 |
| `source_session_id` | `TEXT` | `NULL`, FK -> `session.id`, `ON DELETE SET NULL` | 来源 Session |
| `source_floor_id` | `TEXT` | `NULL`, FK -> `floor.id`, `ON DELETE SET NULL` | 来源 Floor |
| `source_page_id` | `TEXT` | `NULL`, FK -> `message_page.id`, `ON DELETE SET NULL` | 来源 Page |
| `domain` | `TEXT` | `NOT NULL` | 派生结果域名，例如 `summary.candidate` |
| `value_json` | `TEXT` | `NOT NULL`, default `'{}'` | JSON 值，默认最大 256 KiB |
...
- 普通索引 `derived_output_project_created_idx(project_id, created_at)`
- 普通索引 `derived_output_project_domain_idx(project_id, domain, created_at)`
- 普通索引 `derived_output_owner_project_idx(owner_account_id, project_id, created_at)`
- 普通索引 `derived_output_source_session_idx(source_session_id, created_at)`
- 普通索引 `derived_output_workspace_created_idx(workspace_id, created_at)`

说明：

- 创建时只能使用 `draft` 或 `published`。
- 状态流转为 `draft -> published -> archived` 或 `draft -> archived`。
- `archived` 后不能再修改 `value_json`。
- deriver 只能修改自己创建的 Derived Output。

## `project_inbox_item`

Project Inbox 条目表。它保存待 owner 决策的建议或通知。接受条目只更新 Inbox 状态，不会自动把负载写入主 Session。

| 列名 | 类型 | 约束/默认值 | 说明 |
| ---- | ---- | ----------- | ---- |
| `id` | `TEXT` | PK | Inbox 条目 ID，格式 `pinbox_` 前缀 |
| `workspace_id` | `TEXT` | `NOT NULL`, FK -> `workspace.id`, `ON DELETE RESTRICT` | 所属 Workspace |
| `project_id` | `TEXT` | `NOT NULL`, FK -> `project.id`, `ON DELETE RESTRICT` | 所属 Project |
| `account_id` | `TEXT` | `NOT NULL`, FK -> `account.id`, `ON DELETE RESTRICT` | Project owner 账号 ID |
| `sender_account_id` | `TEXT` | `NOT NULL`, FK -> `account.id`, `ON DELETE RESTRICT` | 发送该 Inbox 条目的账号 |
| `type` | `TEXT` | `NOT NULL` | 条目类型，例如 `derived_output.review` |
| `title` | `TEXT` | `NULL` | 可选标题 |
| `payload_json` | `TEXT` | `NOT NULL`, default `'{}'` | JSON 负载，默认最大 256 KiB |
| `source_event_id` | `TEXT` | `NULL`, FK -> `project_event.id`, `ON DELETE SET NULL` | 来源 Project Event |
| `source_session_id` | `TEXT` | `NULL`, FK -> `session.id`, `ON DELETE SET NULL` | 来源 Session |
| `source_floor_id` | `TEXT` | `NULL`, FK -> `floor.id`, `ON DELETE SET NULL` | 来源 Floor |
| `source_page_id` | `TEXT` | `NULL`, FK -> `message_page.id`, `ON DELETE SET NULL` | 来源 Page |
| `status` | `TEXT` | `NOT NULL`, default `pending` | 状态 |
| `decided_by_account_id` | `TEXT` | `NULL`, FK -> `account.id`, `ON DELETE SET NULL` | 决策账号 |
...
- 普通索引 `project_inbox_project_status_created_idx(project_id, status, created_at)`
- 普通索引 `project_inbox_project_created_idx(project_id, created_at)`
- 普通索引 `project_inbox_sender_project_idx(sender_account_id, project_id, created_at)`
- 普通索引 `project_inbox_workspace_created_idx(workspace_id, created_at)`

说明：

- 状态流转为 `pending -> accepted`、`pending -> rejected`、`pending -> archived`、`accepted -> archived`、`rejected -> archived`。
- 只有 owner 可以决定 Inbox 条目。
- deriver 可以创建 Inbox 条目，并且只能读取自己发送的 Inbox 条目。
- observer 不能读取或写入 Inbox。
...
- `actor_account_id` 的派生规则是：当 `actor_type = 'account'` 且
  `actor_id` 非空时优先使用 `actor_id`；否则回退到 `account_id`；
  都没有时保持 `NULL`。
