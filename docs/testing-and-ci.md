...
另外，仓库还需要在 `Settings -> Actions -> General -> Workflow permissions` 里开启 `Allow GitHub Actions to create and approve pull requests`。如果不打开这一项，workflow 会停在“创建同步 PR”这一步。
