...
本文用于记录 TavernHeadless 本轮依赖升级的分批顺序、目标版本、风险点、验证要求，以及收口后的最终结果。

本文最初基于 2026-05-01 的仓库状态和上游版本信息整理。截止 2026-05-02，本轮分批升级已经完成。下文保留执行前的判断依据，并在文首补充实际执行结果。

---

## 已执行结论（2026-05-02）

本轮依赖升级已经按批次完成，并已合并到 `dev`。执行中固定保持了以下目标线：
...
| 批次 | 状态 | 实际结果 |
| ---- | ---- | ---- |
| 0 | 已完成 | 备份数据库文件，并记录基线验证结果 |
| 1 | 已完成 | 升级 Node.js、`better-sqlite3` 和 Fastify 基础依赖，并通过完整回归 |
| 2 | 已完成 | 将 `zod` 固定到 `3.25.76`，并通过完整回归 |
| 3 | 已完成 | 将 `@modelcontextprotocol/sdk` 升级到 `1.29.0`，并通过 API 回归 |
| 4A | 已完成 | 完成 AI SDK `4.x -> 5.x` 过渡，并通过本地回归与 CI |
| 4B | 已完成 | 完成 AI SDK `5.x -> 6.x` 迁移，并显式验证 `openai-compatible` |
| 5 | 已完成 | 将 `@fastify/cors` 升级到 `11.2.0`，现有 CORS 插件实现无需修改 |
| 6 | 已完成 | 将 `drizzle-orm` 升到 `0.45.2`、`drizzle-kit` 升到 `0.31.10`，现有代码无需兼容改动 |

### 当前最终版本线

| 依赖 | 当前版本 | 结果 |
| ---- | ---- | ---- |
| Node.js | `22.22.2 LTS` | 达成固定目标 |
| `better-sqlite3` | `12.8.0` | 达成固定目标 |
| `fastify` | `5.8.5` | 已完成 |
| `@fastify/swagger` | `9.7.0` | 已完成 |
| `@fastify/swagger-ui` | `5.2.6` | 已完成 |
| `@fastify/sensible` | `6.0.4` | 已完成 |
| `@fastify/cors` | `11.2.0` | 已完成 |
| `zod` | `3.25.76` | 达成固定目标 |
| `@modelcontextprotocol/sdk` | `1.29.0` | 已完成 |
| `ai` | `6.0.173` | 已完成 |
| `@ai-sdk/openai` | `3.0.57` | 已完成 |
| `@ai-sdk/anthropic` | `3.0.74` | 已完成 |
| `drizzle-orm` | `0.45.2` | 已完成 |
| `drizzle-kit` | `0.31.10` | 已完成 |

### 收口说明

- `pnpm-lock.yaml` 已与最终版本线同步。
- 本机执行依赖安装与验证时，统一使用本地 Node `22.22.2`。
- 本地 Node 路径为 `.limcode/tmp/node-v22.22.2-win-x64`。
- AI SDK 5 和 6 的迁移中，已按需查阅官方文档。
- `openai-compatible` 路径已做显式验证。
- 当前仍有两个未纳入本轮处理范围的非阻塞事项：
  - VitePress / DocSearch 的 peer warning：`@docsearch/react` 仍声明 `react < 19`
  - GitHub Actions 中个别 Node 20 deprecation warning
