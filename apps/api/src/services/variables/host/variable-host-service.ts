...
import { SessionBranchRegistryService } from "./session-branch-registry-service.js";
...
export class VariableHostService {
  private readonly branchRegistry: SessionBranchRegistryService;

  constructor(private readonly db: AppDb | DbExecutor) {
    this.branchRegistry = new SessionBranchRegistryService(db);
  }
  ...
  private async resolveBranchTargetByParts(
    ...
    const branch = this.branchRegistry.get(accountId, sessionId, branchId);
  ...
  private async resolveFloorTarget(accountId: string, floorId: string): Promise<VariableTarget> {
    const row = await this.db
      .select({
        ...
        floorCreatedAt: floors.createdAt,
        floorUpdatedAt: floors.updatedAt,
        sessionId: sessions.id,
    ...
    this.branchRegistry.ensure({
      accountId,
      sessionId: floor.sessionId,
      branchId: floor.branchId,
      createdAt: floor.floorCreatedAt,
      updatedAt: floor.floorUpdatedAt,
    });
  ...
  private async resolvePageTarget(accountId: string, pageId: string): Promise<VariableTarget> {
    ...
    this.branchRegistry.ensure({
      ...
      createdAt: page.floorCreatedAt,
      updatedAt: page.floorUpdatedAt,
    });
