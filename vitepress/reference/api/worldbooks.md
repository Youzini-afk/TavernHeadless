...
世界书是一组由关键词触发的背景条目。聊天时，如果消息里出现了某个关键词，对应的世界书条目就会被自动注入到提示词里。

每个世界书可以包含很多条条目。世界书数据通常从 SillyTavern 导入。
...
```bash
# 列出所有世界书
curl http://localhost:3000/worldbooks

# 查看某个世界书里的条目
curl "http://localhost:3000/worldbooks/wb_001/entries"
```
...
| 词 | 这里的意思 |
| ---- | ---- |
| 条目 | 世界书里的一条记录，包含触发关键词和要注入的文本 |
| 触发 | 当聊天消息里出现条目设置的关键词时，这条条目就会被激活 |
| 注入位置 | 条目内容被插入到提示词的哪个位置（开头、结尾等） |
