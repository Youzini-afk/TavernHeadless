...
当前处于阶段三：已经有 Project Event、Project observer、Project deriver、Derived Output 和 Project Inbox，但仍不开放完整 Workspace 成员体系，也不开放 Project CRUD 管理 API。
...
| `project` visibility | owner、observer 和 deriver 都可见的 Project Event。 |
...
- client identity 或 API Key identity。
- 插件安装、插件启用系统。
- editor、admin 等新的 Project 角色。
...
| `role` | string | 当前请求账号在该 Project 中的角色：`owner`、`observer` 或 `deriver` |
| `settings_override` | object | Project 设置覆盖。当前暂不开放完整管理能力 |
...
| 类型 | 说明 |
| ---- | ---- |
| `derived_output.created` | 创建 Derived Output |
| `derived_output.updated` | 更新 Derived Output |
| `derived_output.archived` | 归档 Derived Output |
| `project_inbox.item.created` | 创建 Inbox 条目 |
| `project_inbox.item.accepted` | 接受 Inbox 条目 |
| `project_inbox.item.rejected` | 拒绝 Inbox 条目 |
| `project_inbox.item.archived` | 归档 Inbox 条目 |

这些事件的 `payload` 只携带 ID、状态、类型和来源引用，不携带完整 `value` 或 `payload` JSON 正文。
...
| 参数 | 类型 | 默认值 | 说明 |
| ---- | ---- | ---- | ---- |
| `role` | string | - | 可选，`owner`、`observer` 或 `deriver` |
...
增加 Project observer 或 deriver。只有 owner 可以调用。不能通过该接口新增 owner。
...
移除 Project observer 或 deriver。只有 owner 可以调用。当前不支持移除 owner。
...
| `400` | `project_member_role_not_supported` | 只支持新增 observer 或 deriver |
| `400` | `project_member_owner_remove_not_supported` | 当前不支持移除 owner |
| `400` | `project_member_owner_role_conflict` | 不能把 owner 当作普通成员加入，或不能把 owner 降级为其他角色 |
...
| `409` | `project_member_role_conflict` | 成员已经存在，但角色不同 |
...
```ts
const projects = await client.projects.list({ role: "deriver" });
...
await client.projects.addMember(
  "proj_main",
  { accountId: "acc_deriver", role: "deriver" },
);
...
const derivedOutput = await client.projects.derivedOutputs.create(
  "proj_main",
  {
...
const inboxItem = await client.projects.inbox.create(
  "proj_main",
  { type: "derived_output.review", payload: { derivedOutputId: derivedOutput.id } },
  { accountId: "acc_deriver" },
);

await client.projects.inbox.accept("proj_main", inboxItem.id, { accountId: "acc_owner" });
```
