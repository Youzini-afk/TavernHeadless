...
- `Runtime`：平台层运行时能力，以及现有 `/sessions/:id/tools/runtime` 这种工具运行时目录。
- `Run`：聊天主链路中的一次业务运行快照。
- `Execution`：运行内部的子级执行记录，例如工具执行记录。

因此，后续如果新增聊天主链路的楼层进度接口、事件或表结构，应优先使用 `run` 命名，例如 `/floors/:id/run`、`floor.run.updated`、`floor_run_state`，而不是新的 `runtime` 命名。
