...
| Project Event 查询和 SSE 包装 | Project CRUD 管理界面 |
| 保留底层请求能力 | |
...
阶段二已经开放 Project 的读取面、Project Event 查询 / SSE 和 observer 成员维护。普通聊天客户端可以继续忽略这组能力；需要跨会话观察 Project 时再使用。
...
权限规则：owner 可以读写 Project 下资源；observer 只能读取。observer 写入会返回 `403 project_access_denied`。非成员会按旧账号隔离规则隐藏资源。Project API 通常返回 `404 project_not_found`，旧资源路由通常返回 `404 not_found`。
